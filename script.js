...
function salvarLembretes(){
  ...
  const lista = Array.from(cards).map((card) => {
    const prazoElemento = card.querySelectorAll('small')[1];
    const prazo = prazoElemento.textContent.replace('Prazo: ', '');
...
function adicionarLembrete() {
  ...
  const novoCard = criarCardLembrete(texto, prioridade , prazo);
...
// ==========================================================
function criarCardLembrete(texto, prioridade, prazo) {
  ...
  const pequenoPrazo = document.createElement('small');
  pequenoPrazo.textContent = `Prazo: ${prazo}`;
  // Junta o parágrafo e o "small" dentro do wrapper de informações
  infoWrapper.append(paragrafo, pequeno, pequenoPrazo);
...
function iniciarPagina() {
  ...
  salvos.forEach((item) => {
    const card = criarCardLembrete(item.texto, item.prioridade, item.prazo);
